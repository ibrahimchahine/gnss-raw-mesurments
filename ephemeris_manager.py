...
class EphemerisManager:
        ...
        if constellations == None:
            ...
            if timestamp_age.days > 0:
                if legacy_systems_only:
                    data_list.append(
                        self.get_ephemeris_dataframe(filepaths["nasa_daily_gps"])
                    )
                    if "R" in constellations:
                        data_list.append(
                            self.get_ephemeris_dataframe(
                                filepaths["nasa_daily_glonass"]
                            )
                        )
                else:
                    data_list.append(
                        self.get_ephemeris_dataframe(filepaths["nasa_daily_combined"])
                    )
            else:
                data_list.append(
                    self.get_ephemeris_dataframe(filepaths["nasa_daily_gps"])
                ...
                if not legacy_systems_only:
                    data_list.append(
                        self.get_ephemeris_dataframe(filepaths["bkg_daily_combined"])
                    )
    ...
    def get_ephemeris_dataframe(self, fileinfo, constellations=None):
        ...
        if constellations:
            data = georinex.load(
                decompressed_filename, use=constellations
            ).to_dataframe()
        else:
